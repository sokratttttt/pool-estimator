...
import { Plus, Trash2, GripVertical, Save, Type, Heading, Table as TableIcon } from 'lucide-react';
...
const SortableSection = ({ section, index, onChange, onDelete }) => {
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        {section.type !== 'signatures_table' && (
                            <select
                                ...
                                onChange={(e) => onChange(index, { ...section, alignment: e.target.value })}
                                className="bg-white/10 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-bright"
                            >
                        ...
                        {section.type === 'heading' && (
                            <select
                                ...
                                onChange={(e) => onChange(index, { ...section, level: parseInt(e.target.value) })}
                                className="bg-white/10 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-bright"
                            >
                    ...
                    {section.type !== 'signatures_table' && (
                        <textarea
                            value={section.text || ''}
                            onChange={(e) => onChange(index, { ...section, text: e.target.value })}
...
export default function ContractEditor({ template, onSave }) {
                ...
                >
                    ...
                    >
                        {sections.map((section, index) => (
                            <SortableSection
                                key={`section-${index}`} // Using index as key is not ideal but acceptable here since we don't have unique IDs for new sections yet
                                index={index}
                                section={section}
