...
export default function LiveSummary({ onExportPDF, onExportExcel }) {
    ...
    const handleExportExcel = useCallback(async () => {
        ...
        try {
            await onExportExcel(items, total);
        ...
        <>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    {renderSection('Подогрев', items.filter(i => i.section === 'Подогрев'))}
                    {renderSection('Закладные', items.filter(i => i.section === 'Оборудование' && i.id.startsWith('part')))}
                    {renderSection('Доп. опции', items.filter(i => i.section === 'Дополнительное оборудование'))}
                </div>
