...
export function useWorksLogic(selection, updateSelection) {
    ...
    const calculateWorkTotal = useCallback((work, quantity) => {
    ...
    const toggleWork = useCallback((workId, work) => {
    ...
    const startEdit = useCallback((workId, currentQuantity) => {
        setEditingWork(workId);
        setEditValue(currentQuantity.toString());
    }, []);

    const saveEdit = useCallback((workId) => {
