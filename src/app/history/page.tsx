...
export default function HistoryPage() {
    ...
    const handleExportExcel = async (estimate: any) => {
        try {
    ...
    const handleExportPDF = async (estimate: any) => {
        try {
            const clientInfo = estimate.selection?.clientInfo || { name: '', phone: '', email: '' };
    ...
    const handleCompare = async () => {
        ...
        setIsComparing(true);
        try {
            const estimatesToCompare = estimates.filter(e => selectedEstimates.includes(e.id));
            await exportToComparisonPDF(estimatesToCompare);
        ...
        <div className="min-h-screen bg-apple-bg-primary">
            <div className="apple-container apple-section">
                ...
                {isLoading ? (
                    ...
                    <div className="max-w-4xl mx-auto space-y-4">
                        <AnimatePresence>
                            {filteredEstimates.map((estimate: any, idx: number) => (
                                <motion.div
                                    key={estimate.id}
                                            ...
                                            <div className="flex items-start justify-between mb-4">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-2">
                                                        {/* Checkbox */}
                                                        <div onClick={(e: React.ChangeEvent<any>) => e.stopPropagation()} className="flex items-center">
                                                            <input
                                                                type="checkbox"
                                                    ...
                                                    <div className="mb-3">
                                                        <select
                                                            ...
                                                            onChange={(e: React.ChangeEvent<any>) => updateEstimate(estimate.id, { status: e.target.value })}
