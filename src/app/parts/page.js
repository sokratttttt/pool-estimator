                        ...
                        <div className="mt-6 pt-4 border-t border-gray-100">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-lg font-bold text-blue-600">
                                    {part.items.reduce((acc, item) => acc + (item.price * item.quantity), 0).toLocaleString('ru-RU')} ₽
                                </span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-500">Итого монтаж:</span>
                                <span className="font-medium">
                                    {part.items.reduce((acc, item) => acc + item.installationPrice, 0).toLocaleString('ru-RU')} ₽
                                </span>
                            </div>
                        </div>
                    </div>
