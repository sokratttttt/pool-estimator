...
function mapEquipmentType(typeStr) {
    const typeMap = {
        'насос': 'pump',
        'фильтр': 'filter',
        'нагрев': 'heater',
 ...
 */
export function applySmartFilters(equipment, filters) {
    let results = equipment;

    // Filter by type
    if (filters.type) {
        results = results.filter(item =>
            item.category?.toLowerCase().includes(filters.type) ||
            item.name?.toLowerCase().includes(filters.type)
        );
    }
    ...
    if (filters.volume) {
        results = results.filter(item => {
            ...
            return !capacity || (capacity >= filters.volume * 0.8 && capacity <= filters.volume * 1.3);
        });
    }
 ...
 */
export function getSearchSuggestions(query, equipment) {
    const suggestions = [];
 ...
 */
export function explainSearch(query, filters, resultsCount) {
    const parts = [];
