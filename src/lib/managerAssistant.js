// Manager Assistant - AI-powered insights and recommendations

/**
 * Analyze deals and generate actionable insights
 */
export function generateManagerInsights(deals, requests) {
    const insights = [];
...
function findStuckDeals(deals) {
    ...
    return deals
        ...
        .map(deal => ({
            ...deal,
            daysStuck: Math.floor((Date.now() - new Date(deal.updated_at)) / (1000 * 60 * 60 * 24))
        }))
        .sort((a, b) => b.daysStuck - a.daysStuck);
}

function analyzeBestContactTimes(deals, requests) {
...
function generatePriorityTasks(deals, requests) {
    const tasks = [];
    ...
    deals.forEach(deal => {
        ...
        if (deal.stage === 'estimate_sent') {
            const sentDaysAgo = Math.floor((Date.now() - new Date(deal.updated_at)) / (1000 * 60 * 60 * 24));
            if (sentDaysAgo >= 3) {
                tasks.push({
                    type: 'follow_up',
                    priority: 'medium',
        ...
        // Close hot deals
        if (deal.probability >= 80 && deal.stage === 'negotiation') {
            tasks.push({
                type: 'close_deal',
                priority: 'high',
    ...
    requests.forEach(req => {
        if (req.status === 'new') {
            const hoursSinceCreated = (Date.now() - new Date(req.created_at)) / (1000 * 60 * 60);
...
function analyzeConversion(deals) {
...
function calculateForecast(deals) {
    ...
    const forecast = activeDeals.reduce((sum, deal) => {
        const probability = deal.probability || 50;
        return sum + (deal.value * probability / 100);
    }, 0);

    const optimistic = activeDeals.reduce((sum, deal) => sum + deal.value, 0);
 ...
 */
export function getDealRecommendations(deal) {
    const actions = [];
    ...
    if (deal.stage === 'negotiation' && daysSinceUpdate > 3 && deal.probability > 70) {
        actions.push({
            type: 'opportunity',
 ...
 */
export function generateDailyDigest(deals, requests) {
    ...
    return {
        ...
        summary: {
            ...
            revenue: closedToday.reduce((sum, d) => sum + (d.value || 0), 0),
