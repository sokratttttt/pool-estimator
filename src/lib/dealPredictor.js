 ...
 */
export function calculateDealProbability(request) {
    const factors = {};
    let totalScore = 0;

    // 1. Response Time Score (0-25 points)
    if (request.created_at) {
        const hoursSinceCreated = (Date.now() - new Date(request.created_at)) / (1000 * 60 * 60);
...
function calculateEngagement(request) {
    let score = 0;
    ...
    if (request.size) score += 3;
    if (request.type) score += 2;

    return Math.min(25, score);
}

function parseBudget(budgetStr) {
    const num = parseFloat(budgetStr.match(/\d+\.?\d*/)[0]);
 ...
 */
export function scoreAllDeals(deals) {
    return deals.map(deal => ({
        ...deal,
        probabilityData: calculateDealProbability(deal)
 ...
 */
export function getDealInsights(scoredDeals) {
    ...
    const avgScore = scoredDeals.reduce((sum, d) => sum + d.probabilityData.score, 0) / scoredDeals.length;
