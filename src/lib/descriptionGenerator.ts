...
function analyzeEstimate(estimate: EstimateData) {
...
function determinePriceSegment(total?: number) {
...
function extractFeatures(estimate: EstimateData): string[] {
...
function fillTemplate(template: string, config: DescriptionConfig) {
    ...
    description = description.replace(/{length}/g, String(config.length || ''));
...
function estimateTimeline(area: number) {
