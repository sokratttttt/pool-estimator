...
interface PresenceState {
    [key: string]: Array<{ user_id: string; online_at: string }>;
}
...
export function ChatProvider({ children }: ChatProviderProps) {
    ...
    const fetchProfiles = async (): Promise<void> => {
        ...
        if (data) {
            const profilesMap = (data as UserProfile[]).reduce<ProfilesMap>((acc, profile) => {
    ...
    const fetchChannels = async (): Promise<void> => {
        try {
            ...
            setChannels((myChannels as Channel[]) || []);
        } catch (error) {
    ...
    const createChannel = async (name: string, participantIds: string[]): Promise<Channel | undefined> => {
        try {
    ...
    const createDirectMessage = async (otherUserId: string): Promise<Channel | undefined> => {
        if (!currentUser) return;
